...
The weights of all alternatives with respect to the evaluation criteria are illustrated in \fref{fig:radar}. These weights are now combined with the weights of the evaluation criteria, creating a total weight $a_j$ for every alternative $A_j$:
\begin{gather*}
    a_j = \sum_{i = 0}^{m} c_i w_i \quad j = 1, \ldots, n
\end{gather*}
where $c_i$ is the weight of a criterion $C_i$ and $w_i$ is the weight of an alternative $A_j$ with respect to that criterion $C_i$. The total weights are calculated for both the architect and developer and are listed in \tref{tab:total:architect} and \tref{tab:total:developer} respectively. 
